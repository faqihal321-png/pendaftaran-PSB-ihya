...
app.get('/login', (req, res) => {
    res.send('<h2>Login Admin</h2><form action="/login" method="POST">User: <input name="user"><br>Pass: <input name="pass" type="password"><br><button>Login</button></form>');
});
...
app.get('/admin', (req, res) => {
    ...
    const data = readData();
    // Tampilan sederhana untuk memastikan tidak ada syntax error
    let html = '<h1>Data Pendaftar</h1><a href="/logout">Logout</a><hr><table border="1"><tr><th>Nama</th><th>WhatsApp</th><th>Detail</th></tr>';
    data.forEach(p => {
        html += `<tr><td>${p.nama}</td><td>${p.whatsapp}</td><td>${JSON.stringify(p)}</td></tr>`;
    });
    html += '</table>';
    res.send(html);
});
